function [FeaIndex,FeaNumCandi,egvec,approx_err] = myMCFS_p(fea,FeaNumCandi,options)
...
switch lower(options.Method)
    case {lower('LASSO_LARs')}
        ...
        %%debug_start
        approx_err = cell(1,length(FeaNumCandi));
        %%debug_end
        ...
        for i = 1:length(FeaNumCandi)
            ...
            %%shuchu
            %%debug start
            %% calculate the approxiage error
            egvec = eigvector;
            eg_err = (Y - fea*egvec).^2;
            approx_err{i} = sqrt(sum(sum(eg_err)));
            %% debug end
